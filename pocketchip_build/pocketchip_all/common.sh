...
export PATH=/test/tmp/pocketchip_tools/chip-mtd-utils/CHIP-mtd-utils-bb-1.5.2-nand/mkfs.ubifs:$PATH
export PATH=/test/tmp/pocketchip_tools/chip-mtd-utils/CHIP-mtd-utils-bb-1.5.2-nand/ubi-utils:$PATH
...
flash_images() {
  ...
  $FEL -v -p write $UBOOTMEMADDR $IMAGESDIR/uboot-$nand_erasesize.bin || RC=1
  $FEL -v -p write $SPLMEMADDR $IMAGESDIR/spl-$nand_erasesize-$nand_writesize-$nand_oobsize.bin || RC=1
  $FEL -v -p write $UBOOTSCRMEMADDR $ubootscr || RC=1
  $FEL -v -p exe $UBOOTMEMADDR || RC=1
